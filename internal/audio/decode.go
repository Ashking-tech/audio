...
func ParseHeader(data []byte) (Metadata, error) {
	...
	header.Channels = int(binary.LittleEndian.Uint16(data[22:24]))
	header.SampleRate = int(binary.LittleEndian.Uint32(data[24:28]))
	header.BitsPerSample = int(binary.LittleEndian.Uint16(data[34:36]))
