...
func TestParseHeader_invalid(t *testing.T) {
	_, err := audio.ParseHeader([]byte("short"))
	if err == nil {
...
func TestReadPCMSamples(t *testing.T) {
	...
	m, _ := audio.ParseHeader(data)
	samples, err := audio.ReadPCMSamples(data, m)
	if err != nil {
		t.Fatal(err)
	}
	if len(samples) != 100 {
...
func TestReadPCMSamples_unsupportedBits(t *testing.T) {
	m := audio.Metadata{BitsPerSample: 24}
	_, err := audio.ReadPCMSamples(nil, m)
	if err == nil {
...
func TestStereoToMono(t *testing.T) {
	...
	mono := audio.StereoToMono(samples, 2)
	if len(mono) != 2 {
...
func TestStereoToMono_alreadyMono(t *testing.T) {
	...
	mono := audio.StereoToMono(samples, 1)
	if len(mono) != 3 {
...
func TestDecodeWav_invalidFile(t *testing.T) {
	_, err := audio.DecodeWav("/nonexistent/file.wav")
	if err == nil {
...
func TestReadAudioFile(t *testing.T) {
	...
	read, err := audio.ReadAudioFile(f)
	if err != nil {
		t.Fatal(err)
	}
	if len(read) == 0 {
