...
func NewServer(db *sql.DB, addr string) *Server {
	tmp := filepath.Join(os.TempDir(), "audio-fingerprint")
	os.MkdirAll(tmp, 0755)
	return &Server{db: db, addr: addr, tmp: tmp}
}

func (s *Server) Start() error {
	mux := http.NewServeMux()
	mux.Handle("/", http.FileServer(http.Dir("webapp/static")))
